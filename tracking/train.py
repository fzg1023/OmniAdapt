...
def parse_args():
    ...
    # for train
    parser.add_argument('--script', type=str,  default='omniadapt', help='training script name')
...
def main():
    ...
    python_exe = sys.executable  # use the same Python interpreter (with conda env)
    if args.mode == "single":
        train_cmd = "%s lib/train/run_training.py --script %s --config %s --save_dir %s --use_lmdb %d " \
                    "--use_wandb %d" \
                    % (python_exe, args.script, args.config, args.save_dir, args.use_lmdb,
                       args.use_wandb)
        if args.script_prv is not None:
            train_cmd += " --script_prv %s --config_prv %s" % (args.script_prv, args.config_prv)
        if args.distill:
            train_cmd += " --distill %d" % args.distill
            if args.script_teacher is not None:
                train_cmd += " --script_teacher %s --config_teacher %s" % (args.script_teacher, args.config_teacher)
    elif args.mode == "multiple":
        if args.nproc_per_node <= 1:
            # Single GPU: skip torchrun entirely to avoid NCCL SIGSEGV on WSL2
            train_cmd = "%s lib/train/run_training.py " \
                        "--script %s --config %s --save_dir %s --use_lmdb %d --use_wandb %d" \
                        % (python_exe, args.script, args.config,
                           args.save_dir, args.use_lmdb, args.use_wandb)
        else:
            train_cmd = "%s -m torch.distributed.run --nproc_per_node %d --master_port %d lib/train/run_training.py " \
                        "--script %s --config %s --save_dir %s --use_lmdb %d --use_wandb %d" \
                        % (python_exe, args.nproc_per_node, random.randint(10000, 50000), args.script, args.config,
                           args.save_dir, args.use_lmdb, args.use_wandb)
        if args.script_prv is not None:
            train_cmd += " --script_prv %s --config_prv %s" % (args.script_prv, args.config_prv)
        if args.distill:
        ...
        train_cmd = "%s -m torch.distributed.run --nproc_per_node %d --master_addr %s --master_port %d --nnodes %d --node_rank %d lib/train/run_training.py " \
                    "--script %s --config %s --save_dir %s --use_lmdb %d --use_wandb %d" \
                    % (python_exe, args.nproc_per_node, args.ip, args.port, args.world_size, args.rank, args.script, args.config,
                       args.save_dir, args.use_lmdb, args.use_wandb)
        if args.script_prv is not None:
            train_cmd += " --script_prv %s --config_prv %s" % (args.script_prv, args.config_prv)
        if args.distill:
    ...
    print("Running command: %s" % train_cmd)
    subprocess.run(train_cmd, shell=True, env=os.environ.copy())
