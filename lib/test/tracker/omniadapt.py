...
class OmniAdapt(BaseTracker):
    ...
    def __init__(self, params):
        ...
        # mmap avoids materializing the checkpoint's optimizer state in RAM.
        # The test path only needs the network weights.
        try:
            checkpoint = torch.load(
                self.params.checkpoint, map_location='cpu', mmap=True,
                weights_only=False)
        ...
        try:
            network.load_state_dict(checkpoint['net'], strict=True, assign=True)
        except TypeError:
