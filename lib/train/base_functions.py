...
def get_optimizer_scheduler(net, cfg):
    ...
    freeze_except = getattr(cfg.TRAIN, 'FREEZE_EXCEPT', None)

    if freeze_except is not None:
        # 冻结模式: 只训练 FREEZE_EXCEPT 指定的模块，其余冻结
        for n, p in net.named_parameters():
            if any(e in n for e in freeze_except):
        ...
        param_dicts = [
            {"params": [p for n, p in net.named_parameters() if p.requires_grad],
             "lr": getattr(cfg.TRAIN, 'FREEZE_LR', cfg.TRAIN.LR)},
        ]
        if is_main_process():
            print(f"FREEZE_EXCEPT mode: only training {freeze_except}")
            for n, p in net.named_parameters():
                if p.requires_grad:
                    print(f"  [trainable] {n}")
    else:
        addtion_param = ['decode', 'sca', 'CSS', 'MCI', 'spatial_aggregator', 'template_query', 'afg']
        param_dicts = [
