...
# Early dependency check — fail fast with a clear message before distributed init
_REQUIRED_MODULES = {
    'cv2':        'opencv-python',
    'torch':      'pytorch',
    'yaml':       'PyYAML',
...
_OPTIONAL_MODULES = {
    'wandb':   'wandb (only needed with --use_wandb 1)',
    'lmdb':    'lmdb (only needed with --use_lmdb 1)',
}
_missing = []
_optional_missing = []
for _mod, _pkg in _REQUIRED_MODULES.items():
    try:
        __import__(_mod)
    except ImportError:
        _missing.append(f"  {_mod} → pip install {_pkg}")
for _mod, _pkg in _OPTIONAL_MODULES.items():
    try:
        __import__(_mod)
    except ImportError:
        _optional_missing.append(f"  {_mod} → pip install {_pkg}")
if _missing:
    sys.exit("ERROR: Missing required packages:\n" + "\n".join(_missing) +
             "\n\nPlease install them and try again.")
if _optional_missing:
    print("WARNING: Some optional packages are missing (training may still work):")
...
def run_training(script_name, config_name, cudnn_benchmark=True, local_rank=-1, save_dir=None, base_seed=None,
    ...
    if script_name_prv is not None and config_name_prv is not None and script_name_prv != 'None' and config_name_prv != 'None':
    ...
    if distill:
        ...
        if script_teacher is not None and config_teacher is not None and script_teacher != 'None' and config_teacher != 'None':
...
def main():
    ...
    parser.add_argument('--script', type=str, default='omniadapt', help='Name of the train script.')
    ...
    # Use LOCAL_RANK env var (set by torchrun) if available, otherwise fall back to --local_rank
    if 'LOCAL_RANK' in os.environ:
        args.local_rank = int(os.environ['LOCAL_RANK'])
    world_size = int(os.environ.get('WORLD_SIZE', '1'))
    # Only init distributed when actually using multiple GPUs (avoids SIGSEGV on WSL2)
    if args.local_rank != -1 and world_size > 1:
        dist.init_process_group(backend='nccl')
        torch.cuda.set_device(args.local_rank)
    else:
        if args.local_rank != -1:
            args.local_rank = -1  # treat single-GPU torchrun as non-distributed
