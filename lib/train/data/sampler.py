...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def getitem(self):
        ...
        while not valid:
            ...
            if is_video_dataset:
                ...
                if self.frame_sample_mode == 'causal' or self.frame_sample_mode == 'sequential':
                    # causal: 模板帧在前，搜索帧在后，间隔 ≤ max_gap
                    # sequential: 连续帧，间隔紧凑（刚好够采样所需帧数）
                    max_gap = max(self.num_search_frames, self.num_template_frames) if self.frame_sample_mode == 'sequential' else self.max_gap
                    while search_frame_ids is None:
                        ...
                        if base_frame_id is None:
                            break
                        prev_gap = max_gap + gap_increase
                        prev_frame_ids = self._sample_visible_ids(visible, num_ids=self.num_template_frames - 1,
                                                                  min_id=base_frame_id[0] - prev_gap,
                                                                  max_id=base_frame_id[0])
                        if prev_frame_ids is None:
                            gap_increase += 5
                            continue
                        template_frame_ids = base_frame_id + prev_frame_ids
                        search_gap = max_gap + gap_increase
                        search_frame_ids = self._sample_visible_ids(visible, min_id=template_frame_ids[0] + 1,
                                                                  max_id=template_frame_ids[0] + search_gap,
    ...
    def sample_seq_from_dataset(self, dataset, is_video_dataset):
        ...
        while not enough_visible_frames:
            ...
            if seq_info_dict is None:
                continue  # skip bad sequences (e.g. missing annotation files)
    ...
    def get_one_search(self):
        ...
        search_frames, search_anno, meta_obj_test = dataset.get_frames(seq_id, search_frame_ids, seq_info_dict)
        # Safety net: retry if sequence data is invalid (e.g. missing files)
        if search_frames is None:
            return self.get_one_search()
