...
class BaseTrainer:
    ...
    def train(self, max_epochs, load_latest=False, fail_safe=True, load_previous_ckpt=False, distill=False):
        ...
        epoch = -1
        num_tries = 2  # allow one retry on fail_safe
        for i in range(num_tries):
            try:
                ...
                break  # training completed successfully, exit retry loop
            except:
    ...
    def load_checkpoint(self, checkpoint = None, fields = None, ignore_fields = None, load_constructor = False):
        ...
        # Warn on net_type mismatch instead of crashing — allows loading compatible checkpoints
        if net_type != checkpoint_dict.get('net_type', ''):
            print("WARNING: net_type mismatch in checkpoint (checkpoint: '%s', current: '%s'). "
                  "Attempting to load anyway..."
                  % (checkpoint_dict.get('net_type', 'unknown'), net_type))
    ...
    def load_state_dict(self, checkpoint=None, distill=False):
        """Loads a network checkpoint file for pretrained weights.
        Unlike load_checkpoint, this does NOT require net_type to match.
        """
        if distill:
        ...
        # Downgrade net_type check to warning for pretrained weights
        if net_type != checkpoint_dict.get('net_type', ''):
            print("WARNING: net_type mismatch (checkpoint: '%s', current: '%s'). "
                  "This is expected when loading pretrained weights from a different architecture."
                  % (checkpoint_dict.get('net_type', 'unknown'), net_type))
